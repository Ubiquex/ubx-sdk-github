# Code generated by `ubx sdk gen` from github_repo_search_result_item_list. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class SearchResultItemList_Items_License:
    html_url: Any = None
    key: Any = None
    name: Any = None
    node_id: Any = None
    spdx_id: Any = None
    url: Any = None

@dataclasses.dataclass
class SearchResultItemList_Items_Owner:
    avatar_url: Any = None
    email: Any = None
    events_url: Any = None
    followers_url: Any = None
    following_url: Any = None
    gists_url: Any = None
    gravatar_id: Any = None
    html_url: Any = None
    id: Any = None
    login: Any = None
    name: Any = None
    node_id: Any = None
    organizations_url: Any = None
    received_events_url: Any = None
    repos_url: Any = None
    site_admin: Any = None
    starred_at: Any = None
    starred_url: Any = None
    subscriptions_url: Any = None
    type: Any = None
    url: Any = None
    user_view_type: Any = None

@dataclasses.dataclass
class SearchResultItemList_Items_Permissions:
    admin: Any = None
    maintain: Any = None
    pull: Any = None
    push: Any = None
    triage: Any = None

@dataclasses.dataclass
class SearchResultItemList_Items_TextMatches_Matches:
    indices: Any = None
    text: Any = None

@dataclasses.dataclass
class SearchResultItemList_Items_TextMatches:
    fragment: Any = None
    matches: Any = None
    object_type: Any = None
    object_url: Any = None
    property: Any = None

@dataclasses.dataclass
class SearchResultItemList_Items:
    allow_auto_merge: Any = None
    allow_forking: Any = None
    allow_merge_commit: Any = None
    allow_rebase_merge: Any = None
    allow_squash_merge: Any = None
    archive_url: Any = None
    archived: Any = None
    assignees_url: Any = None
    blobs_url: Any = None
    branches_url: Any = None
    clone_url: Any = None
    collaborators_url: Any = None
    comments_url: Any = None
    commits_url: Any = None
    compare_url: Any = None
    contents_url: Any = None
    contributors_url: Any = None
    created_at: Any = None
    default_branch: Any = None
    delete_branch_on_merge: Any = None
    deployments_url: Any = None
    description: Any = None
    disabled: Any = None
    downloads_url: Any = None
    events_url: Any = None
    fork: Any = None
    forks: Any = None
    forks_count: Any = None
    forks_url: Any = None
    full_name: Any = None
    git_commits_url: Any = None
    git_refs_url: Any = None
    git_tags_url: Any = None
    git_url: Any = None
    has_discussions: Any = None
    has_downloads: Any = None
    has_issues: Any = None
    has_pages: Any = None
    has_projects: Any = None
    has_pull_requests: Any = None
    has_wiki: Any = None
    homepage: Any = None
    hooks_url: Any = None
    html_url: Any = None
    id: Any = None
    is_template: Any = None
    issue_comment_url: Any = None
    issue_events_url: Any = None
    issues_url: Any = None
    keys_url: Any = None
    labels_url: Any = None
    language: Any = None
    languages_url: Any = None
    license: Any = None
    master_branch: Any = None
    merges_url: Any = None
    milestones_url: Any = None
    mirror_url: Any = None
    name: Any = None
    node_id: Any = None
    notifications_url: Any = None
    open_issues: Any = None
    open_issues_count: Any = None
    owner: Any = None
    permissions: Any = None
    private: Any = None
    pull_request_creation_policy: Any = None
    pulls_url: Any = None
    pushed_at: Any = None
    releases_url: Any = None
    score: Any = None
    size: Any = None
    ssh_url: Any = None
    stargazers_count: Any = None
    stargazers_url: Any = None
    statuses_url: Any = None
    subscribers_url: Any = None
    subscription_url: Any = None
    svn_url: Any = None
    tags_url: Any = None
    teams_url: Any = None
    temp_clone_token: Any = None
    text_matches: Any = None
    topics: Any = None
    trees_url: Any = None
    updated_at: Any = None
    url: Any = None
    visibility: Any = None
    watchers: Any = None
    watchers_count: Any = None
    web_commit_signoff_required: Any = None

@dataclasses.dataclass
class SearchResultItemListConfig:
    order: Any = None
    page: Any = None
    per_page: Any = None
    q: Any = None
    sort: Any = None

@dataclasses.dataclass
class SearchResultItemListAttrs:
    incomplete_results: Any = None
    items: Any = None
    order: Any = None
    page: Any = None
    per_page: Any = None
    q: Any = None
    sort: Any = None
    total_count: Any = None

SearchResultItemList = ubx.DataSourceBinding(
    wire_type="github_repo_search_result_item_list",
    fields={
        "order": ubx.FieldSpec(wire_name="order"),
        "page": ubx.FieldSpec(wire_name="page"),
        "per_page": ubx.FieldSpec(wire_name="per_page"),
        "q": ubx.FieldSpec(wire_name="q"),
        "sort": ubx.FieldSpec(wire_name="sort"),
    },
)
